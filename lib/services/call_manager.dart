...
import 'package:swarapp/app/configs.dart';
...
class CallManager {
  ...
  void _initCalls() {
    ...
    _callClient!.onReceiveNewSession = (callSession) async {
      if (_currentCall != null && _currentCall!.sessionId != callSession.sessionId) {
        callSession.reject();
        return;
      }
  ...
  void startNewCall(BuildContext context, int callType, Set<int> opponents) {
    ...
    //   ),
    // );
    // Get.to(() => ConversationCallScreen(callSession, false));
    preferencesService.addAudioCallOverlay(context, callSession, false);
  ...
  void acceptCall(String sessionId) {
    ...
    if (_currentCall != null) {
      if (context != null) {
        ...
        preferencesService.addAudioCallOverlay(context!, _currentCall!, true);
      }
    } else {
  ...
  Future<CreateEventParams> _getCallEventParameters(P2PSession currentCall) async {
    // String? callerName = users.where((cubeUser) => cubeUser.id == currentCall.callerId).first.fullName;
    var result = await getUserById(currentCall.callerId);
    String? callerName = result!.fullName;
  ...
  void _sendStartCallSignalForOffliners(P2PSession currentCall) async {
    CreateEventParams params = await _getCallEventParameters(currentCall);
  ...
  void _sendEndCallSignalForOffliners(P2PSession currentCall) async {
