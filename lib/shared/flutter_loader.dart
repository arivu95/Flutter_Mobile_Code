...
class ProgressLoader extends StatelessWidget {
  ...
  ProgressLoader._(this._progressIndicator, this._themeData);
          ...
          children: <Widget>[
            SafeArea(
              child: Container(
                ...
                margin: EdgeInsets.only(top: !isAppbarOverlay ? overlayFromTop ?? defaultValue : 0.0, bottom: isBottomBarOverlay ? 0.0 : overlayFromBottom ?? defaultValue),
              ),
            ),
            Center(
                child: ProgressLoader._(
              progressIndicator,
              themeData,
            )),
          ],
  ...
  Widget build(BuildContext context) {
    return Center(
        child: Theme(
            data: _themeData ?? Theme.of(context).copyWith(colorScheme: ColorScheme.fromSwatch().copyWith(secondary: activeColor)),
            child: HeartbeatProgressIndicator(
                child: _progressIndicator ??
                    Image.asset(
                      'assets/progress_loading.gif',
                      width: 70,
                      height: 70,
                    ))));
  }
