...
class SpanCalculator {
  ...
  static DateDuration timeToNextSubscription(DateTime birthdate, {DateTime? fromDate}) {
    DateTime endDate = fromDate ?? DateTime.now();
    DateTime tempDate = DateTime(endDate.year, birthdate.month, birthdate.day);
    DateTime nextBirthdayDate = tempDate.isBefore(endDate) ? SpanCalculator.add(date: tempDate, duration: DateDuration(years: 1)) : tempDate;
    return dateDifference(fromDate: endDate, toDate: nextBirthdayDate);
  }

  static DateTime nextBirthDate(DateTime birthdate, {DateTime? fromDate}) {
    DateTime endDate = fromDate ?? DateTime.now();
    DateTime tempDate = DateTime(endDate.year, birthdate.month, birthdate.day);
    DateTime nextBirthdayDate = tempDate.isBefore(endDate) ? SpanCalculator.add(date: tempDate, duration: DateDuration(years: 1)) : tempDate;
