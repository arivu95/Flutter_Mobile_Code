import 'package:doctor_module/src/ui/doc_onboarding/doctor_notification_model.dart';
...
class DoctorNotificationView extends StatefulWidget {
  DoctorNotificationView({Key? key}) : super(key: key);

  @override
  _DoctorNotificationViewState createState() => _DoctorNotificationViewState();
}

class _DoctorNotificationViewState extends State<DoctorNotificationView> {
...
//getNotifications
  ...
  void check_accept(BuildContext context, DoctorNotificationmodel model, String id, int index) async {
    Loader.show(context);
    var getRes = await model.acceptinvite(id, "accepted");
    if (getRes == 200) {
      Loader.hide();
  ...
  Widget notificationList(dynamic notifInfo, DoctorNotificationmodel model, int index) {
    return Container(
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton(
                  onPressed: () async {
                    Get.back(result: {'refresh': false});
                    await model.declineinvite(notifInfo["_id"], "declined");
              ...
              ElevatedButton(
                  onPressed: () async {
                    check_accept(context, model, notifInfo["_id"], index);
  ...
  setViewedState() {}
  Widget list_of_Notif(BuildContext context, DoctorNotificationmodel model) {
    return Expanded(
        child: Container(
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: Container(
            ...
            width: Screen.width(context),
            child: ViewModelBuilder<DoctorNotificationmodel>.reactive(
                ...
                viewModelBuilder: () => DoctorNotificationmodel())),
      ),
    );
  }
}
