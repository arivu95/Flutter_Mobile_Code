import 'package:documents_module/src/ui/medical_records/recentdocument_widget/recent_doc_widgetmodel.dart';
...
class RecentDocumentWidget extends StatefulWidget {
  final String categoryId;
//  RecentDocumentWidget({Key? key, required this.categoryId}) : super(key: key);
  RecentDocumentWidget({Key? key, required this.categoryId}) : super(key: key);

  @override
  RecentDocumentWidgetState createState() => RecentDocumentWidgetState();
}

class RecentDocumentWidgetState extends State<RecentDocumentWidget> {
  @override
  ...
  RecentDocViewmodel modelRef = RecentDocViewmodel();
...
//*****************for recent upload documents
  ...
  Widget build(BuildContext context) {
    if (locator<PreferencesService>().isReload.value == true) {
      ...
      modelRef.getRecentUploads(widget.categoryId);
      // modelRef.getRecentUploads();
    }
    return ViewModelBuilder<RecentDocViewmodel>.reactive(
        onModelReady: (model) {
          model.init();
          model.getRecentUploads(widget.categoryId);
          // modelRef.getRecentUploads();
          modelRef = model;
        },
                        ...
                        GestureDetector(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text('Recent Documents').fontWeight(FontWeight.w500).bold(),
                              ],
                            )),
        ...
        viewModelBuilder: () => RecentDocViewmodel());
  }
}
